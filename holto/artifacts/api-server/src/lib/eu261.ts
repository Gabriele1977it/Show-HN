// Deterministic UK261 / EU261 rights engine — the authoritative, no-network
// computation behind HOLTO's honesty promise. The legal content (eligibility,
// amounts, thresholds, checklist) is computed here, never invented by an LLM.
//
// NOTE: the airport table + calcEU261 mirror the client's `utils/eu261.ts`
// (used for the on-screen compensation card). Keep the two in sync; a shared
// workspace lib is the eventual home.
...
// Resolve a user-entered origin/destination (IATA code OR city name) to an
// airport record. The wizard accepts either form.
export function resolveAirport(input: string): { name: string; city: string; lat: number; lon: number } | null {
  const q = input.trim().toUpperCase();
  if (AIRPORTS[q]) return AIRPORTS[q];
  const byCity = Object.values(AIRPORTS).find((a) => a.city.toUpperCase() === q || a.name.toUpperCase() === q);
  return byCity ?? null;
}
...
export function calcEU261(distKm: number, disruptionType: DisruptionKind, delayHours?: number): EU261Result {
  ...
  if (disruptionType === "missed_connection") {
    return {
      ...
      note: "",
      reason:
        "Missed connections may be covered depending on whether they were booked as a single ticket. Use the delay or cancellation calculator for that leg.",
    };
  }
  ...
  return { eligible: true, amount, reducedAmount: amount / 2, tier, distKm, note };
}
